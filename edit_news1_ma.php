<?php
    ...
    if(isset($_POST["info"])){
        ...
        $date = $_POST["date"];
        include "dblink.php";
        $sql = "UPDATE `news1` SET `picture` = '$file', `info` = '$info', `date` = '$date' WHERE `news1`.`aid` = $id";
        $result = mysqli_query($link, $sql);
        echo "<script>alert('您已修改此筆消息資料!'); location.href = 'news1_ma.php';</script>";
        mysqli_close($link);
        }
